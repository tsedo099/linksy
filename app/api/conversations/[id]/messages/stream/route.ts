...
import {
  getConversationMessageBus,
  type ConversationMessageEvent,
} from "@/lib/conversation-message-bus";
...
// GET …/messages/stream — SSE: conversation changed (Redis pub/sub + same-process fan-out).
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const bus = getConversationMessageBus();
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const unsubscribe = bus.subscribe(conversationId, (event: ConversationMessageEvent) => {
